...
const techIconMap = {
  ...
  redux: "https://cdn.iconscout.com/icon/free/png-256/free-redux-logo-icon-svg-download-png-3030243.png",
  ...
  streamlit: "https://images.seeklogo.com/logo-png/44/2/streamlit-logo-png_seeklogo-441815.png",
...
const getProjectIcons = (project) => {
  ...
  return getTechList(project).map((tech) => getTechIcon(tech)).filter(Boolean);
};
    ...
    <section className="projects-section section-reveal py-20 px-4 md:px-12 lg:px-24" id="projects">
      ...
      {sectionsToShow.map((section, idx) => (
        <div key={`${section.heading || "section"}-${section.subHeading || "main"}-${idx}`} className="mb-14">
          ...
          {section.subHeading && (
            <h3 className="text-lg text-[var(--text-muted)] mb-3 font-medium">{section.subHeading}</h3>
          )}
              ...
              >
                <div className="p-6 flex flex-col h-full">
                  <div className="font-bold text-lg text-white mb-2 flex items-center">
                    {project.title}
                    {project.org && <span className="ml-2 font-normal italic text-[var(--text-muted)]">({project.org})</span>}
                  </div>
                  ...
                  {project.liveUrl && (
                    <div className="mb-2 text-sm">
                      <strong className="text-[var(--electric-cyan)]">Live:</strong>{" "}
                      <a
